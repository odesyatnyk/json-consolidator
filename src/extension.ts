...
export function activate(context: vscode.ExtensionContext) {
	...
	const supportedDirectories = config.get<Array<string>>(
		"supportedDirectories"
	);
	const triggerEverywhere = config.get<boolean>("triggerEverywhere");
	const suppressInitNotification = config.get<boolean>(
		"suppressInitNotification"
	);
	const targetEditorWindow = config.get<string>("targetEditorWindow") || "Two";

	vscode.commands.executeCommand(
		"setContext",
		"ext.supportedDirectories",
		supportedDirectories
	);
	vscode.commands.executeCommand(
		"setContext",
		"ext.triggerEverywhere",
		triggerEverywhere
	);

	let disposable = vscode.commands.registerCommand(
		"json-consolidator.editor",
		(path: vscode.Uri) => {
			const panel = vscode.window.createWebviewPanel(
				"json-consolidator",
				`Json Consolidator - ${getLastSegment(path)}`,
				getViewColumn(targetEditorWindow),
				{
					retainContextWhenHidden: true,
					enableScripts: true,
					localResourceRoots: [
						vscode.Uri.file(pathJoin(context.extensionPath, "ui")),
					],
				}
			);
	...
	if (!suppressInitNotification) {
		vscode.window
			.showInformationMessage(
				"Json Consolidator is ready to rock 🚀",
				DONT_SHOW_AGAIN
			)
			.then((action) => {
